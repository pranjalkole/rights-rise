...
function App() {
  ...
  if (state == State.Normal) {
    return (
      <>
        ...
        <>
          <h1>Your chats</h1>
          {chats.length != 0 ? chats.map((chat) => <a href="#" onClick={() => {setChat(chat); setState(State.Chat); setMessages([]);}}>{chat}</a>) : <p>You have no chats</p>}
        </> }
        <Footer />
      </>
    )
  ...
  return (
    <>
      <Header signedIn={signedIn} handleLogout={handleLogout} />
      <a href="#" onClick={() => setState(State.Normal)}>Back</a>
      <h1>{chat}</h1>
      <p>Messages</p>
      {messages.length != 0 ? messages.map((message) => <p>{message}</p>) : <p>No messages</p>}
